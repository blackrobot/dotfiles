...
2. `.zshrc` - Interactive shell config, loads Antidote plugins, aliases
3. `.zsh_plugins.txt` - Antidote plugin list (powerlevel10k, autosuggestions, completions, forgit, etc.)
...
Plugins are managed via Antidote (replaced zplug). Key annotations in `.zsh_plugins.txt`:
- `kind:fpath` - Add to fpath for completions only (lightweight)
- `kind:defer` - Defer loading until after prompt (requires zsh-defer)
- `kind:path` - Add to PATH instead of sourcing
- `path:subdir` - Load from subdirectory (used for oh-my-zsh plugins)

Deferred plugins load ~10ms after the prompt appears, improving startup time.
...
2. Installs Antidote (plugin manager, auto-installed on first shell if missing)
