...
prompt_git() {
  ...
  if $(git rev-parse --is-inside-work-tree >/dev/null 2>&1); then
    ...
    vcs_info
    echo -n "%{%B%}${ref/refs\/heads\// }${vcs_info_msg_0_}%{%b%}"
    if [[ -n $dirty ]]; then
      prompt_segment yellow white
    else
      prompt_segment green white
...
prompt_dir() {
  prompt_segment blue white "%{%B%}${PWD/#$HOME/~}%{%b%}"
...
prompt_time() {
  local the_date the_time icon
  icon="%{%F{white}%}⌚ %{%f%}"
  the_date="%{%F{blue}%}%D{%F}%{%f%}"
  the_time="%{%F{blue}%}%*%{%f%}"
  echo -n "%{%B%}$the_date $icon $the_time%{%b%}"
}
